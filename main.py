...
def login():
    ...
    if request.method == 'POST' and str(request.form['username']) !="" and str(request.form['password']) != "":
        ...
        result = executeselectquery("SELECT firstname,lastname,email  FROM users WHERE username  ='"+username+"' AND password_name = '"+password+"'" )
        if (result.shape[0] != 0):
...
def registration():
    ...
    if request.method == 'POST' and str(request.form['username']) !="" and str(request.form['password']) !="" and str(request.form['firstname']) !="" and str(request.form['lastname']) !="" and str(request.form['email']) !="":
        ...
        if (result.shape[0] != 0):
            ...
            result1 = cursor.execute("""INSERT INTO users (username, password_name, firstname, lastname, email) values (%s, %s, %s, %s, %s)""", (username, password, firstname, lastname, email))
            conn.commit()
            cursor.close()
            conn.close()
            result2 = execute_query( "SELECT firstname,lastname,email  FROM users WHERE username  = '"+username+"' AND password_name ='"+ password+"'")
            if result2:
...
def connecttoDataBase():
    # your MySQl DB Config parameters to be passes here
    config = {
